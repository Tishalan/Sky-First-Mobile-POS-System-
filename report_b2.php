...
$branch_id = 2; // Hardcoded for Branch 2
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate_report'])) {
    if ($from_date) {
        // Base where clause
        $where_clause = "WHERE branch_id = '$branch'";
        if ($to_date) {
        ...
        while ($bill = $bills_result->fetch_assoc()) {
            ...
            while ($item = $items_result->fetch_assoc()) {
                ...
                $profit_ratio = ($item['price'] - $item['original_price']) / $item['price'];
                $profit = ($payment_method == 'Credit') ? ($effective_item_amount * $profit_ratio) - ($item['discount'] * ($effective_item_amount / $item_total)) : ($item['price'] - $item['original_price']) * $item['quantity'] - $item['discount'];
                $total_profit += $profit;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['export_excel'])) {
    ...
    header('Content-Disposition: attachment;filename="sales_report_b2.csv"');
...
<head>
    ...
    <title>Reports - SKY FIRST MOBILE Billing (Branch 2)</title>
...
<body>
    <div class="header">
        ...
        <div class="header-content">
            <h1><i class="fas fa-mobile-alt"></i> SKY FIRST MOBILE Billing (Branch 2)</h1>
            <div class="date-time">
    ...
    <div class="section">
        <h2><i class="fas fa-chart-pie"></i> Sales Reports (Branch 2)</h2>
